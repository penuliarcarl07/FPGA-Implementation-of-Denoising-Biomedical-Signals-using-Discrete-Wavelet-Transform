// -------------------------------------------------------------
// 
// File Name: hdl_prj\hdlsrc\db2_level5_final_hdlsss\Threshold_Estimator.v
// Created: 2024-04-18 06:57:11
// 
// Generated by MATLAB 9.10 and HDL Coder 3.18
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Threshold_Estimator
// Source Path: db2_level5_final_hdlsss/DWT_db2_5_level/Threshold_Estimator
// Hierarchy Level: 1
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Threshold_Estimator
          (clk,
           reset,
           enb,
           Signal_w,
           Threshold);


  input   clk;
  input   reset;
  input   enb;
  input   signed [15:0] Signal_w;  // sfix16_En27
  output  [15:0] Threshold;  // ufix16_En27


  wire [15:0] Absolute_Value_out1;  // ufix16_En28
  wire [15:0] Median_out1;  // ufix16_En28
  reg [15:0] Median_out1_1;  // ufix16_En28
  wire [15:0] Length_out1;  // ufix16_En14
  reg [15:0] Length_out1_1;  // ufix16_En14
  wire [31:0] Multiply_mul_temp;  // ufix32_En42
  wire [15:0] Multiply_out1;  // ufix16_En27
  reg [15:0] Multiply_out1_1;  // ufix16_En27


  Absolute_Value u_Absolute_Value (.clk(clk),
                                   .reset(reset),
                                   .enb(enb),
                                   .Signal_w(Signal_w),  // sfix16_En27
                                   .Out_Abs(Absolute_Value_out1)  // ufix16_En28
                                   );
  Median u_Median (.clk(clk),
                   .reset(reset),
                   .enb(enb),
                   .In_Abs(Absolute_Value_out1),  // ufix16_En28
                   .Median_1(Median_out1)  // ufix16_En28
                   );
  always @(posedge clk)
    begin : HwModeRegister_process
      if (reset == 1'b1) begin
        Median_out1_1 <= 16'b0000000000000000;
      end
      else begin
        if (enb) begin
          Median_out1_1 <= Median_out1;
        end
      end
    end



  assign Length_out1 = 16'b1001001100111111;


  always @(posedge clk)
    begin : HwModeRegister1_process
      if (reset == 1'b1) begin
        Length_out1_1 <= 16'b0000000000000000;
      end
      else begin
        if (enb) begin
          Length_out1_1 <= Length_out1;
        end
      end
    end



  assign Multiply_mul_temp = Median_out1_1 * Length_out1_1;
  assign Multiply_out1 = Multiply_mul_temp[30:15];


  always @(posedge clk)
    begin : PipelineRegister_process
      if (reset == 1'b1) begin
        Multiply_out1_1 <= 16'b0000000000000000;
      end
      else begin
        if (enb) begin
          Multiply_out1_1 <= Multiply_out1;
        end
      end
    end



  assign Threshold = Multiply_out1_1;

endmodule  // Threshold_Estimator

